...
class ProfileDetailsTableViewCell: UITableViewCell {
    ...
    func bindData(){
        nameLbl.text = "Name : " + ( DataStore.shared.userAuth?.fullName?.description ?? "-")
        emailLbl.text = "Email : " + ( DataStore.shared.userAuth?.email?.description ?? "-")
        phoneNumberLbl.text = "Phone Number : " + ( DataStore.shared.userAuth?.mobilePhone?.description ?? "")
